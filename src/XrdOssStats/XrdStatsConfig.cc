                                ...
                                XrdOucEnv    *envP)
{
    ...
    if (native_oss->Init(logger, config_fn, envP) != 0) {
        log.Emsg("Initialize", "Stats filesystem failed to initialize the underlying native storage.");
        return nullptr;
    }
...
XrdVERSIONINFO(XrdOssGetStorageSystem,fsstats);
XrdVERSIONINFO(XrdOssGetStorageSystem2,fsstats);
