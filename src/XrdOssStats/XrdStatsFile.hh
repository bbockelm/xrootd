...
class StatsFile : public XrdOssDF {
public:
    StatsFile(const char *user, std::unique_ptr<XrdOssDF> ossDF, XrdSysError &log, StatsFileSystem &oss) :
      XrdOssDF(user),
    ...
    ssize_t ReadV(XrdOucIOVec *readV, int rdvcnt) override
    {
        ...
        auto result = m_wrapped->ReadV(readV, rdvcnt);
